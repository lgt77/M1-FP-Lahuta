...
public class Cryptor {
    public String cypher(String input, int key) {
        ...
        for(char symbol : charsArray) {
            if(ConstantsForCryptor.ALPHABET.getCharsArrayConstants().contains(Character.toUpperCase(symbol))){
                sb.append(symBel.symbolsBelongingABC(symbol, rotator.rotateAlphabet(key)));
            } else if (ConstantsForCryptor.PUNCTUATION.getCharsArrayConstants().contains(symbol)) {
